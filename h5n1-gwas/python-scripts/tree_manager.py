...
def init_pickled_trees(output_folder_path, gene = None):
    gene = gene or cfg.gene
    json_tree_path = output_folder_path + "/pickled_json_tree.obj"
    with open(json_tree_path, 'rb') as pickled_json_tree:
        json_tree = pickle.load(pickled_json_tree)
    baltic_tree_path = output_folder_path + "/pickled_baltic_tree.obj"
    with open(baltic_tree_path, 'rb') as pickled_baltic_tree:
        orig_tree = pickle.load(pickled_baltic_tree)
    no_muts_tree = get_no_muts_tree(orig_tree, gene)
    pickled_tree = pickle.dumps(no_muts_tree, -1)
    return json_tree, orig_tree, no_muts_tree, pickled_tree
